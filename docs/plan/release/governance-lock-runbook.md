...
Nova targets complete public SDK parity for Python, TypeScript, and R.
`generated-clients` enforces canonical OpenAPI, committed Python SDK drift, and
retained TS/R scaffold drift. `typescript-clients` and `r-clients` remain
required validation lanes for those retained foundations.
