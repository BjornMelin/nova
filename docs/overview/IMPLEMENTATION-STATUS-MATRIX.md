...
| SDK layout | canonical TypeScript package is `packages/nova_sdk_ts` published as `@nova/sdk`; canonical Python package is `packages/nova_sdk_py` published/imported as `nova-sdk-py` / `nova_sdk_py`; TS generation uses `@hey-api/openapi-ts`, Python generation uses `openapi-python-client`, and only the R package rename remains on a later branch | one package per language, Hey API / openapi-python-client / httr2 | `ADR-0037`, `SPEC-0030`, `docs/clients/CLIENT-SDK-CANONICAL-PACKAGES.md` |
