...
| Release workflows | GitHub workflow-driven selective versioning, immutable API Lambda zip publication in `release-apply`, repo-owned runtime deployment via `deploy-runtime`, authoritative `deploy-output.json` evidence, staged package publish, prod promotion, and deploy-output-driven validation | repo-owned CodePipeline/CodeBuild runtime promotion control plane, manual/external runtime deployment, free-text URL validation authority, and CDK-local API packaging | `docs/runbooks/release/README.md`, `docs/runbooks/release/release-runbook.md`, `docs/contracts/workflow-deploy-runtime-v1.schema.json`, `docs/contracts/deploy-output-authority-v2.schema.json`, `infra/nova_cdk/README.md` |
