...
| Transfer control plane | direct-to-S3 uploads with additive initiate hints, DynamoDB-backed upload sessions, AppConfig-backed effective transfer policy resolution with environment fallback, deterministic quota enforcement, and dedicated export-copy tuning controls | opaque browser defaults with no durable upload-session state or scoped quota enforcement | `SPEC-0002`, `SPEC-0005`, `SPEC-0027`, `README.md` |
| AWS runtime | Regional REST API + one canonical custom domain via `infra/nova_cdk`, with FastAPI served through the repo-owned Lambda entrypoint (Mangum-backed, zip-packaged, Python 3.13, arm64); WAF is enabled by default in prod and disabled by default in non-prod unless explicitly enabled; Storage Lens, transfer budget notifications, and scheduled multipart reconciliation are part of the active operational surface | ECS/Fargate runtime stacks, CloudFront API ingress, Lambda Web Adapter, uvicorn-in-Lambda, and worker deploy control plane | `ADR-0033`, `SPEC-0029`, `infra/nova_cdk/README.md` |
