...
| AWS runtime | Regional REST API + direct Regional WAF + one canonical custom domain via `infra/nova_cdk` | ECS/Fargate runtime stacks, CloudFront API ingress, and worker deploy control plane | `ADR-0033`, `SPEC-0029`, `infra/nova_cdk/README.md` |
...
| Release workflows | GitHub workflow-driven selective versioning, staged publish, prod promotion, and route validation | repo-owned CodePipeline/CodeBuild runtime promotion control plane | `docs/runbooks/release/README.md`, `docs/runbooks/release/release-runbook.md` |
