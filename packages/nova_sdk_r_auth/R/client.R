...
nova_auth_default_request_performer <- function(request) {
  if (!is.list(request)) {
  ...
  encoded_body <- nova_auth_encode_request_body(request$body, request$content_type)
...
nova_auth_request_descriptor <- function(
  client,
  operation_id,
  body = NULL,
  path_params = NULL,
  ...
  path_params <- nova_auth_normalize_named_list(path_params, "path_params")
  query <- nova_auth_normalize_named_list(query, "query")
  request_headers <- nova_auth_normalize_named_list(headers, "headers")
  ...
  merged_headers <- nova_auth_prune_null_headers(merged_headers)
  resolved_path <- nova_auth_resolve_path(operation$path, path_params)
  selected_content_type <- nova_auth_select_content_type(operation, content_type)
  if (is.null(body)) {
...
nova_auth_execute_operation <- function(
  client,
  operation_id,
  body = NULL,
  path_params = NULL,
  ...
  request <- nova_auth_request_descriptor(
    client = client,
  ...
  if (ok) {
    data <- nova_auth_parse_json_body(body_text)
    error <- NULL
  } else {
    data <- NULL
    error <- nova_auth_decode_error_envelope(body_text, status = status)
  }
  list(
    ok = ok,
    status = status,
...
create_nova_auth_client <- function(
  base_url,
  request_performer = nova_auth_default_request_performer,
  default_headers = NULL,
  timeout_seconds = 30
) {
  base_url <- nova_auth_normalize_base_url(base_url)
  ...
  default_headers <- nova_auth_normalize_named_list(default_headers, "default_headers")
  ...
  class(client) <- c("nova_auth_client", "list")
  client
}
