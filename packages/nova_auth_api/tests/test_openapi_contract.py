...
def test_openapi_path_method_tags_are_semantic() -> None:
    """
    Verify that the OpenAPI document groups authentication-related endpoints under semantic tags.
    
    Fetches /openapi.json and asserts the path->method->tags mapping matches the expected tags for health and token endpoints.
    """
    app = create_app()
    with TestClient(app) as client:
        response = client.get("/openapi.json")
