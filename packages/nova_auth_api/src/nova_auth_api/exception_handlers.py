...
def register_exception_handlers(app: FastAPI) -> None:
    """
    Register canonical exception handlers on the FastAPI application.
    
    Registers handlers for:
    - AuthApiError: returns the exception's canonical error payload, status code, and headers.
    - RequestValidationError: returns a 422 canonical error payload with validation errors sanitized to redact sensitive input.
    - Exception: returns a canonical internal error payload for unexpected failures.
    """
...
def sanitize_validation_errors(
    *,
    ...
    """
    Redact sensitive input values in a sequence of validation error objects.
    
    Parameters:
        errors (Sequence[Any]): Sequence of validation error mappings; each mapping may include a `"loc"` location and an `"input"` value.
    
    Returns:
        list[dict[str, Any]]: A list of validation error dictionaries where any `"input"` whose `"loc"` indicates a sensitive field is replaced with `"[REDACTED]"`.
    """
