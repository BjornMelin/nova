...
def install_auth_openapi(app: FastAPI) -> None:
    ...
    def customize_openapi(schema: dict[str, Any]) -> None:
        """
        Customize the given OpenAPI schema with auth-specific components, responses, and path-level adjustments.
        
        This mutates the provided schema in-place to:
        - Ensure component objects and request schemas for token introspection exist.
        - Register a bearerAuth security scheme.
        - Ensure canonical error envelope and error response components with prescribed descriptions.
        - Apply operation-level response references and replace validation error responses with the canonical AuthInvalidRequestResponse.
        - Mark introspection POST operations with an `x-auth-not-required` hint and attach a request body for /v1/token/introspect.
        - Prune unused validation-error schemas.
