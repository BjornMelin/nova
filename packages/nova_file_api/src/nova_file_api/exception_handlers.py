...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def request_validation_error_handler(
        ...
        """
        Handle request validation errors by returning a canonical HTTP 422 JSON error response.
        
        Returns:
            JSONResponse: HTTP 422 response with canonical error content containing code "invalid_request", message "request validation failed", `details` with sanitized validation errors under `errors`, and the request ID.
        """
