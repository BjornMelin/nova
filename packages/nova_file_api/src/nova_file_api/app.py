...
def create_app(*, container_override: AppContainer | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        """
        Application lifespan context that attaches configuration and a dependency container to the FastAPI app and ensures any container authenticator is closed on shutdown.
        
        If a container_override is provided, the override is attached to app.state and its authenticator (if any) is closed when the application stops. Otherwise, AWS clients/resources are created, a container is instantiated and attached to app.state for the application lifetime; the container's authenticator (if any) is closed on shutdown.
        """
