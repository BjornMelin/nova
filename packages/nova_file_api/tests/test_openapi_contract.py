...
def _build_openapi_app() -> FastAPI:
    """
    Create a FastAPI application configured for OpenAPI/schema regression tests.
    
    The application is wired with in-memory and test-double components (in-memory job repository,
    memory activity store, stub transfer service, local and shared caches, metrics) and overrides
    external integrations so no real AWS clients or external services are started.
    
    Returns:
        app (FastAPI): A FastAPI application instance ready for generating OpenAPI schema and
        exercising route-related tests.
    """
    settings = Settings()
...
def _operation_id_map(payload: dict[str, Any]) -> dict[str, dict[str, str]]:
    """
    Extracts operationId values for allowed HTTP methods from an OpenAPI payload's paths.
    
    Parameters:
        payload (dict[str, Any]): Parsed OpenAPI document containing a "paths" mapping.
    
    Returns:
        operation_map (dict[str, dict[str, str]]): Mapping from path to a mapping of HTTP method to its `operationId`.
    
    Raises:
        AssertionError: If the payload structure is not as expected or an `operationId` is missing or not a non-empty string.
    """
