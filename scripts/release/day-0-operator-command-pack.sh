...
DEPLOY_IMAGE_DIGEST_VARIABLE="${DEPLOY_IMAGE_DIGEST_VARIABLE:-}"
...
if [ -z "$DEPLOY_IMAGE_DIGEST_VARIABLE" ]; then
  case "$NOVA_DEPLOY_SERVICE_NAME" in
    nova-file-api)
...
echo "Deploy image digest variable: $DEPLOY_IMAGE_DIGEST_VARIABLE"
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    DeployImageDigestVariable="$DEPLOY_IMAGE_DIGEST_VARIABLE" \
