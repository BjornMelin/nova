...
def _render_r_client(target: GenerationTarget) -> str:
    ...
    lines = [
        ...
        f"    user_agent = {prefix}_null_coalesce(user_agent, {prefix}_default_user_agent())",
        "  )",
...
def _render_r_namespace(target: GenerationTarget) -> str:
    ...
    lines = [
        ...
        "",
        "S3method(conditionMessage,nova_file_api_error)",
        "",
    ]
...
def _render_r_package_manual(target: GenerationTarget) -> str:
    ...
    lines.extend(
        [
            ...
            f"\\description{{{target.r_package_description}}}",
            "\\usage{",
            f'  {constructor}(base_url, bearer_token = NULL, bearer_token_env = "NOVA_FILE_BEARER_TOKEN", default_headers = NULL, timeout_seconds = 30, user_agent = NULL)',
            f'  {bearer_helper}(token = NULL, env_var = "NOVA_FILE_BEARER_TOKEN")',
            "}",
            "",
            "\\arguments{",
...
def _render_r_tests(
    ...
    if prefix == "nova_file":
        ...
        lines = [
            'test_that("generated auth package exports thin endpoint wrappers", {',
            '  exports <- getNamespaceExports("nova.sdk.r.auth")',
            '  expect_true("nova_auth_verify_token" %in% exports)',
            '  expect_false("nova_auth_request_descriptor" %in% exports)',
            "})",
            "",
        ]
    return "\n".join(lines)
